...
type ApiFormGroupContent = {
  ...
  hash: FormControl<IApi['hash']>;
  urlBase: FormControl<IApi['urlBase']>;
  token: FormControl<IApi['token']>;
};
...
export class ApiFormService {
  createApiFormGroup(api: ApiFormGroupInput = { id: null }): ApiFormGroup {
    ...
    return new FormGroup<ApiFormGroupContent>({
      ...
      hash: new FormControl(apiRawValue.hash, {
        validators: [Validators.required],
      }),
      urlBase: new FormControl(apiRawValue.urlBase),
      token: new FormControl(apiRawValue.token),
    });
  }
